...
public class OrderDispatchFunctions extends LogicUtil {
  ...
  public void changePrices(OrderDataBean orderDataBean, boolean billPeriodChanged)
    ...
    for (VOTEMPORD voTempOrd : vTempOrd) {
      /*
       * Case 28260
       */
      if (voTempOrd.getRXNUMBER().length() == 0
          && (!billPeriodChanged || billPeriodChanged && voTempOrd.getITEMTYPE().startsWith("R"))) {
        this.changePricesByItem(orderDataBean, voTempOrd, "", 0);
  ...
  public void packageDefaultPrice(PriceUtil price, String sDefaultPrice, int packageId)
    ...
    if (isNotBlank(sDefaultPrice)) {
      ...
      price.setDailyRate(Double.parseDouble(sDefaultPrice));
      price.setWkRate(Double.parseDouble(sDefaultPrice));
      price.setMonRate(Double.parseDouble(sDefaultPrice));

      if (new BigDecimal(sDefaultPrice).compareTo(BigDecimal.ZERO) > 0 && packageId > 0) {
        Formatter f = new Formatter();
        f.format("SELECT DRATE, WRATE, MRATE FROM PKHDR WITH(NOLOCK)");
        f.format(" WHERE PKHDRSEQ = %s", packageId);
        RowSet rs = dao.Exce_Select(f.toString());
        if (rs.next()) {
          price.setUnitPrice(Double.parseDouble(sDefaultPrice));
          price.setSalePrice(Double.parseDouble(sDefaultPrice));
          price.setDailyRate(rs.getDouble("DRATE"));
          price.setWkRate(rs.getDouble("WRATE"));
          price.setMonRate(rs.getDouble("MRATE"));
        }
      }
    }
  }
